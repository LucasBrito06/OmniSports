...
// Initialize a map layer for markers
...
function addMarkers(locations) {
    ...
    locations.forEach(location => {
        const marker = L.marker(location.position, {
            title: location.name // Display name on hover
        });

        // Optional: Add a popup for more details
        marker.bindPopup(`
            <b>${location.name}</b><br>
            Type: ${location.type}<br>
            City: ${location.city}<br>
            Sports: ${location.sports.join(', ') || 'None'}
        `);
...
function filterLocations(searchQuery) {
    ...
    // Apply 200km radius for filtered locations after search
    ...
    ).filter(location => userLocation ? getDistance(userLocation, location.position) <= 200 : true);
...
async function fetchDataFromFirebase() {
    ...
    if (userLocation) {
        // Initially filter locations within a 10km radius
        sportsLocations = fetchedLocations.filter(loc => getDistance(userLocation, loc.position) <= 10);
    } else {
