...
// Display user's location on the map and add a marker
if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(position => {
        ...
        if (userLocationMarker) {
            map.removeLayer(userLocationMarker); // Remove existing marker if present
        }
        ...
        fetchDataFromFirebase(); // Load data without user location
    });
} else {
    fetchDataFromFirebase(); // Load data without geolocation
}
...
navigator.geolocation.getCurrentPosition(position => {
    ...
    if (userLocationMarker) map.removeLayer(userLocationMarker); // Remove previous marker if exists
    userLocationMarker = L.circleMarker(userLocation, {
        radius: 8,
        color: 'blue',
        fillColor: '#007bff',
        fillOpacity: 0.8
    }).addTo(map).bindPopup("Voce");

    // Fetch data from Firebase
    fetchDataFromFirebase();
}, error => {
    console.error("Could not retrieve user location:", error.message);
    alert("Could not get your location. Defaulting to all locations.");
    fetchDataFromFirebase();
});
