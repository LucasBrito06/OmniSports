...
function filterLocations(searchQuery) {
    ...
    // Filter locations based on the search query without a radius limit
    ...
    addMarkers(filteredLocations); // Add markers for the search results
}
...
async function fetchDataFromFirebase() {
    ...
    if (userLocation) {
        // Filter locations within 5km radius for the static map view
        const nearbyLocations = fetchedLocations.filter(loc => getDistance(userLocation, loc.position) <= 5);
        addMarkers(nearbyLocations); // Display only nearby markers on the static map
    } else {
        addMarkers(fetchedLocations); // If no user location, display all markers by default
    }
}
